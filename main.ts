/**
 * IPAM-NAM-NSG-SSI Main Entry Point
...
const start = async (): Promise<void> => {
  try {
    ...
    if (Deno.env.get("CRON_MODE") !== "true") {
      logger.info(
        `ipam-nam-nsg-ssi: Initializing worker on ${Deno.hostname()}`,
      );
      await ssiWorker.work();
      logger.debug(
        `ipam-nam-nsg-ssi: Waiting to flush logs before exiting.`,
      );
      Deno.exit(0);
    } else {
      logger.info(
        `ipam-nam-nsg-ssi: Initializing worker on ${Deno.hostname()} running every ${SSI_INTERVAL} seconds...`,
      );
      await ssiWorker.work();
    ...
    if (error instanceof Error) {
      logger.error(
        `ipam-nam-nsg-ssi: Error occurred on ${Deno.hostname()}, ${error.message}`,
        {
          component: "main",
          method: "start",
      ...
      logger.error(
        `ipam-nam-nsg-ssi: Unknown error occurred on ${Deno.hostname()}`,
        {
          component: "main",
          method: "start",
          error: error,
